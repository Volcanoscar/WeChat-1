...
public class WeiXmppManager {
	 ...
	 * @return
	 */
	public boolean isBeingRegistered(){
		return registerFlag;
	}
...
//			XMPPConnection.DEBUG_ENABLED = true;
				...
				for (int i = 0; i < CONNECT_TIME; i++) {
					...
					try {
						...
						if (connect != null && connect.isConnected()) {
							...
							Log.i(TAG,
									"send access info:" + userLoginIQ.toXML());
							return null;
						}
					} catch (XMPPException e) {
						// TODO Auto-generated catch block
	...
	private void startLogin() {
		...
		try {
			...
			// connection.connect();
			connection = getXMPPConnection("124.251.36.70", 5223);
			connection.connect();

			if (connection != null && connection.isConnected()) {
				Log.d(TAG, "start login connectted!!");
				...
				String content = "<auth xmlns=\"tcl:hc:portal\">"
						...
						+ "<resource>tv-android-wechat</resource>" + "<token>"
						+ token + "</token>" + "</auth>";
				...
				try {
					if (packageIq instanceof LSMemIDResultIQ) {
						...
						if (memberId != null
							...
							if (connect != null) {
								Log.i(TAG,
										"Register finished, connect.disconnect");
								connect.disconnect();
							}
			...
			public void processPacket(Packet packet) {
				...
				if (packageIq instanceof LoginResultIQ) {
					...
					if (ReturnType.STATUS_SUCCESS.equals(errorCode)) {
						...
						//离线消息初始化
						InitFinish initfinish = new InitFinish(connection,
								 null); initfinish.sentPacket();
